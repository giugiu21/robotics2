...
%%
%setting up the parameters given
ddr_d = [-1;-1];
q = [3*pi/2, pi/4, 0];
l = [0.5, 0.5, 0.5];
...
I_xx_2 = (1/40)*m2;
I_xx_3 = (1/40)*m3;
I_yy_2 = (1/12)*m2*(0.05+l2^2);
I_yy_3 = (1/12)*m3*(0.05+l3^2);
I_zz_2 = (1/12)*m2*(0.05+l2^2);
I_zz_3 = (1/12)*m3*(0.05+l3^2);

M = double(subs(M, [q1, q2, q3, l1, l2, l3, dc1, dc2, dc3, m1, m2, m3, Ic_yy_1, Ic_xx_1, Ic_xx_2, Ic_xx_3, Ic_yy_2, Ic_yy_3, Ic_zz_2, Ic_zz_3], ...
    [3*pi/2, pi/4, 0, 0.5, 0.5, 0.5, 0.25, 0.25, 0.25, 2, 2, 2, I_yy_1, I_xx_1, I_xx_2, I_xx_3, I_yy_2, I_yy_3, I_zz_2, I_zz_3]));

disp("Inertia Matrix with values given")
disp(M)
J = double(subs(J, [q1, q2, q3, l1, l2, l3], [3*pi/2, pi/4, 0, 0.5, 0.5, 0.5]));

disp("Task Jacobian with values given")
disp(J)
disp("Jacobian has rank: (if==2 full rank)")
disp(rank(J))

%My optimization problem that was given is
M_2 = inv(M*M);

ddq_d  = M_2* J'*inv(J*M_2*J')*ddr_d;
