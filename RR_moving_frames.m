...
w1 = transpose(R1)*(w0+(1-s1)*dq1*[0;0;1]); 
simplify(w1);
disp(w1)
...
v1 = transpose(R1)*(v0+(s1)*dq1*[0; 0; 1]+cross(w1,[l1*cos(q1);l1*sin(q1);0]));
simplify(v1);
disp(v1)
...
T1 = (1/2)*m1*transpose(vc1)*vc1 + (1/2)*I1zz*transpose(w1)*w1;
simplify(T1);
disp(T1)
...
%% compute angular velocity w of link 1
w2 = transpose(R2)*(w1+(1-s2)*dq2*[0;0;1]);
simplify(w2);
disp(w2)
...
%% compute linear velocity v of link 1
v2 = transpose(R2)*(v1+(s2)*dq1*[0; 0; 1]+cross(w2,[l2*cos(q2);l2*sin(q2);0]));
simplify(v2);
disp(v2)
...
T2 = (1/2)*m2*transpose(vc2)*vc2 + (1/2)*I2zz*transpose(w2)*w2;
simplify(T2);
disp(T2)
pause
